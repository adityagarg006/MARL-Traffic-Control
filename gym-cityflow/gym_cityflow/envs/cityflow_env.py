...
class Cityflow(gym.Env):
    ...
    def __init__(self, configPath, episodeSteps):
        ...
        self.graph_neighbors = {i: [] for i in self.intersectionNames}
        for road in self.roadnetDict['roads']:
            u = road.get('startIntersection')
            v = road.get('endIntersection')
            # only include real intersections
            if u in self.graph_neighbors and v in self.graph_neighbors:
                # undirected
                self.graph_neighbors[u].append(v)
                self.graph_neighbors[v].append(u)
    ...
    def step(self, action):
        ...
        #reward
        self.reward = self.getReward_collab()
    ...
    def getReward_collab(self):
        waiting = self.eng.get_lane_waiting_vehicle_count()
        W = {}
        for i in self.intersectionNames:
            W[i] = sum(
                waiting.get(ln,0)
                for grp in self.intersections[i][1]
                for ln in grp
            )
            
        shaped = []
        for i in self.intersectionNames:
            neigh_sum = sum(W[n] for n in self.graph_neighbors[i])
            ri = -(W[i] + self.alpha * neigh_sum)
            shaped.append([i, ri])
        return shaped
            ...
            for i in range(len(self.intersections[key][1])):
                ...
                for j in range(len(self.intersections[key][1][i])):
                    ...
                    for k in range(len(vehicle)):
                        ...
                        if self.vehicle_speeds[vehicle[k]] < 0.5:
